...
function isVideoReady() {
    // p5 createCapture returns a p5.Element; the underlying DOM video element is at .elt
    if (!videoElement || !videoElement.elt) return false;
    const v = videoElement.elt;
    // readyState >= 2 (HAVE_CURRENT_DATA) or non-zero videoWidth indicate the video is ready
    return (typeof v.readyState === 'number' && v.readyState >= 2) || (v.videoWidth && v.videoWidth > 0);
}